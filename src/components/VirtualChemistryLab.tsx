...
const validateReaction = (subA: string, subB: string | null, method: Method): ReactionResult => {
  ...
  if (method === 'dpnc') {
    if (subA === 'Al2O3' && subB === 'Criolit') {
        return {
          success: true,
          message: 'Điện phân nóng chảy $Al_2O_3$ với Criolit tạo ra Nhôm và khí Oxy.',
    ...
    if (['MgCl2', 'NaCl'].includes(subA)) {
      ...
      if (subA === 'MgCl2') {
        return {
          success: true,
          message: 'Điện phân nóng chảy $MgCl_2$ tạo ra Magie và khí Clo.',
  ...
  if (method === 'dpdd') {
      ...
      if (subA === 'CuCl2') {
        return {
          success: true,
          message: 'Điện phân dung dịch $\\mathrm{CuCl}_2$ tạo ra Đồng và khí Clo.',
      ...
      if (subA === 'CuSO4') {
        return {
          success: true,
          message: 'Điện phân dung dịch $\\mathrm{CuSO}_4$ tạo ra Đồng, khí Oxy và Axit Sunfuric.',
          equation: '$2\\mathrm{CuSO}_4 + 2\\mathrm{H}_2\\mathrm{O} \\xrightarrow{\\mathrm{đpdd}} 2\\mathrm{Cu} + \\mathrm{O}_2\\uparrow + 2\\mathrm{H}_2\\mathrm{SO}_4$',
          animationType: 'electro',
      ...
      if (subA === 'AgNO3') {
        return {
          success: true,
          message: 'Điện phân dung dịch $\\mathrm{AgNO}_3$ tạo ra Bạc, khí Oxy và Axit Nitric.',
          equation: '$4\\mathrm{AgNO}_3 + 2\\mathrm{H}_2\\mathrm{O} \\xrightarrow{\\mathrm{đpdd}} 4\\mathrm{Ag} + \\mathrm{O}_2\\uparrow + 4\\mathrm{HNO}_3$',
          animationType: 'electro',
          products: [
      ...
      if (subA === 'NaCl') {
        return {
          success: true,
          message: 'Điện phân dung dịch $\\mathrm{NaCl}$ có màng ngăn tạo ra $\\mathrm{NaOH}$, Hydro và Clo.',
          equation: '$2\\mathrm{NaCl} + 2\\mathrm{H}_2\\mathrm{O} \\xrightarrow{\\mathrm{đpdd, màng ngăn}} 2\\mathrm{NaOH} + \\mathrm{H}_2\\uparrow + \\mathrm{Cl}_2\\uparrow$',
          animationType: 'electro',
          products: [
  ...
  if (method === 'nl') {
    ...
    if (['Fe2O3', 'CuO'].includes(subA)) {
      ...
      if (subA === 'Fe2O3') {
        return {
          success: true,
          message: `Khử $Fe_2O_3$ (đỏ nâu) bằng ${chemB.name} tạo ra $Fe$ (xám trắng).`,
  ...
  if (method === 'tl') {
    ...
    if (subA === 'CuSO4') {
      if (['Fe', 'Zn', 'Al'].includes(subB!)) {
        return {
          success: true,
          message: `${chemB.name} đẩy Đồng ra khỏi dung dịch $\\mathrm{CuSO}_4$.`,
          equation: `$\\mathrm{${subB}} + \\mathrm{CuSO}_4 \\rightarrow \\mathrm{${subB}SO}_4 + \\mathrm{Cu}\\downarrow$`,
          animationType: 'hydro',
    ...
    if (subA === 'AgNO3') {
      if (['Fe', 'Zn', 'Cu'].includes(subB!)) {
        return {
          success: true,
          message: `${chemB.name} đẩy Bạc ra khỏi dung dịch $\\mathrm{AgNO}_3$.`,
          equation: `$\\mathrm{${subB}} + n\\mathrm{AgNO}_3 \\rightarrow \\mathrm{${subB}}(\\mathrm{NO}_3)_n + n\\mathrm{Ag}\\downarrow$`,
          animationType: 'hydro',
            ...
            <div className="mb-6">
              ...
              <div className="grid grid-cols-3 gap-2">
                {Object.values(CHEMICALS).filter(c => c.type === 'reactant').map(chem => (
                  <div
                    key={chem.id}
                    onClick={() => labState === 'idle' && setSubA(chem.id)}
                    role="button"
                    tabIndex={0}
                    className={cn(
                      "p-2 rounded-lg border text-sm font-medium transition-all text-center cursor-pointer",
                      labState !== 'idle' ? "opacity-50 pointer-events-none" : "",
            ...
            <div>
              ...
              <div className="grid grid-cols-3 gap-2">
                {Object.values(CHEMICALS).filter(c => c.type === 'catalyst').map(chem => (
                  <div
                    key={chem.id}
                    onClick={() => labState === 'idle' && setSubB(chem.id)}
                    role="button"
                    tabIndex={0}
                    className={cn(
                      "p-2 rounded-lg border text-sm font-medium transition-all text-center cursor-pointer",
          ...
          <div className="bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-3xl p-8 shadow-2xl relative min-h-[500px] flex flex-col items-center justify-center overflow-hidden">
            ...
            <div className="absolute top-6 right-6 flex gap-3 z-30">
              {labState === 'idle' ? (
                ...
                <button
                  onClick={handleReset}
                  className="px-6 py-3 bg-slate-200 dark:bg-slate-800 hover:bg-slate-300 dark:hover:bg-slate-700 text-slate-900 dark:text-white font-bold rounded-xl transition-colors flex items-center gap-2"
                >
            ...
            <div className="absolute bottom-6 left-1/2 -translate-x-1/2 z-30 bg-white/80 dark:bg-slate-900/80 backdrop-blur px-6 py-2 rounded-full border border-slate-200 dark:border-slate-700 shadow-sm">
              <span className="text-slate-800 dark:text-white font-mono text-sm uppercase tracking-widest">
